...
class RSGDcOptimizer(optimizer.Optimizer):
    def __init__(self, learning_rate=0.5, m=0.5, c=1, D=1, use_locking=False, name="RSGDc"):
        super(RSGDcOptimizer, self).__init__(use_locking, name)
    ...
    def _apply_dense(self, grad, var):
        ...
        a = tf.divide(p, tf.sqrt(tf.multiply(p, p) / tf.pow(c_t, 2) + tf.pow(m_t, 2)))
        b = lr_t * (grad + D_t * a)
        p_t = p.assign(p - tf.reshape(b, tf.shape(p)))
        ### Update theta: theta_(t+1) = theta_t + lr * p_(t+1) / sqrt(p_(t+1) * p_(t+1) / c^2 + m^2)
        ### ALSO COMPONENTWISE
        g_t = - tf.divide(p_t, tf.sqrt(tf.multiply(p_t, p_t) / tf.pow(c_t, 2) + tf.pow(m_t, 2)))
...
tf.train.RSGDcOptimizer = RSGDcOptimizer
